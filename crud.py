...
def look_up(query):
    # models.init_db()
    return look_up_esp(query)

def look_up_esp(query):
    models.init_db()
    try:
...
def insert(word, definition, part_of_speech="", frequency=0, lang="esp"):
    if lang == "esp":
        results = models.EsperantoEntry.select(models.Esperanto.word.lower() == word.lower())
        try:
            result = results.get()
...
def update(id, definition, part_of_speech, frequency, lang="esp"):
    if lang == "esp":
        try:
